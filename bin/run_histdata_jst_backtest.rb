...
options = {
  ...
  timeframe: 'M1',
  debug_mode: false,
  convert_to_jst: true  # 日本時間への変換をデフォルトで有効に
}

OptionParser.new do |opts|
  opts.banner = "使用法: ruby run_backtest_jst.rb [options]"
  
  opts.on("-m", "--mq4 FILE", "MT4ファイルのパス") do |file|
    options[:mq4_file] = file
  end
  
  opts.on("-d", "--data FILE", "HistDataのCSVファイルパス") do |file|
  ...
  opts.on("-j", "--[no-]jst", "日本時間に変換する (デフォルト: 有効)") do |v|
    options[:convert_to_jst] = v
  end
...
if options[:data_file].nil?
  puts "エラー: ヒストリカルデータファイルのパスを指定してください (-d または --data オプション)"
  exit 1
end
...
puts "Trevianバックテスト実行（HistDataの#{options[:convert_to_jst] ? '日本時間調整' : 'EST時間'}データを使用）"
puts "==========================================="
puts "設定:"
puts "- MT4ファイル: #{options[:mq4_file]}"
puts "- ヒストリカルデータ: #{options[:data_file]}"
...
puts "- 日本時間変換: #{options[:convert_to_jst] ? '有効' : '無効'}"
...
# データ読み込み（タイムゾーンオプション付き）
tick_data = data_loader.load(options[:data_file], {
  format: :specific,
  date_format: '%Y.%m.%d',
  convert_to_jst: options[:convert_to_jst]  # 日本時間への変換オプション
})
...
if options[:chart_file]
  ...
  timezone_info = options[:convert_to_jst] ? "（日本時間）" : "（EST時間）"
  chart_title = "Trevian バックテスト結果 (#{options[:symbol]} #{options[:timeframe]}) #{timezone_info}"
